...
def generate_mrn():
    ...
    if latest_user and latest_user.mrn:
        try:
            new_count = int(latest_user.mrn.split('-')[-1]) + 1
        except ValueError:
            new_count = 1
    else:
        new_count = 1
...
def generate_booking_id():
    ...
    latest_appt = Appointment.query.filter(
        Appointment.booking_order_id.like(f"{prefix}%")
    ).order_by(Appointment.booking_order_id.desc()).first()

    if latest_appt and latest_appt.booking_order_id:
        try:
            new_count = int(latest_appt.booking_order_id.split('-')[-1]) + 1
        except ValueError:
            new_count = 1
    else:
        new_count = 1
