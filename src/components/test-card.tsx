...
export function TestCard({ id, name, description, price, tests, isSelected, onSelect, buttonText }: TestCardProps) {
  return (
    <Card
      className={`group relative overflow-hidden rounded-3xl border cursor-pointer transition-all duration-500 ${
        isSelected 
    ...
    >
      ...
      <CardHeader className="pb-4 relative z-10 pt-8">
        <div className="flex justify-between items-start gap-4">
          <CardTitle className="leading-tight text-xl font-bold dark:text-white group-hover:text-blue-700 dark:group-hover:text-blue-400 transition-colors">{name}</CardTitle>
          {isSelected && <CheckCircle2 className="w-6 h-6 text-blue-500 shrink-0 drop-shadow-sm animate-in zoom-in" />}
        </div>
        <CardDescription className="line-clamp-2 mt-2 dark:text-slate-400">{description || "Comprehensive diagnostic panel for full evaluation."}</CardDescription>
      </CardHeader>

      <CardContent className="space-y-6 relative z-10">
        <div className="flex items-baseline gap-1 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/50 transition-colors group-hover:bg-blue-50/50 dark:group-hover:bg-blue-900/20">
        ...
        {onSelect && (
          <Button 
            className={`w-full rounded-xl py-6 font-bold shadow-none transition-all duration-300 ${
              isSelected 
